...
from ostatslib.actions.clustering import KMeansClustering
...
def test_small_deviation_data_yields_positive_reward(
    ...
    reward = KMeansClustering().execute(dummy_small_deviation_blobs_data,
                                        state,
                                        DEFAULT_CONFIG)[1]
    assert reward >= 0.6
...
def test_big_deviation_data_yields_negative_reward(
    ...
    state.set("response_variable_label", '')
    state.set("clusters_count", N_CLUSTERS)
    reward = KMeansClustering().execute(dummy_big_deviation_blobs_data,
                                        state,
                                        DEFAULT_CONFIG)[1]
    assert reward < 0
