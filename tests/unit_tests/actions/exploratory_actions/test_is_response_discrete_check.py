...
"""
is_response_discrete_check action testing module
"""
...
def test_is_response_discrete_check_yields_negative_rewards_if_feature_is_known(
        dummy_data: DataFrame) -> None:
    """
    is_response_discrete_check should yield positive rewards if the info is unkown.
    Once the info is known and the state is updated, \
    any other attempt to run the same action should yield a negative reward
    """
    state = State()
    state, reward, _ = is_response_discrete_check(state, dummy_data)
    assert reward > 0
    assert state.get("is_response_discrete") == -1

    reward = is_response_discrete_check(state, dummy_data)[1]
    assert reward < 0
