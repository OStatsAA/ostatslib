...
def model_mock() -> Mock:
    ...
    attrs = {
        ...
        'predict.return_value': TEST_ACTION_NAME
    }
...
def test_agent_memory(dummy_state: State) -> None:
    ...
    agent.remember_transition(dummy_state, TEST_ACTION_NAME, dummy_state, .42)
...
def test_agent_calls_model_fit_on_update(model_mock: Mock) -> None:
    ...
    model_mock.fit.assert_called_once_with(memory)
...
def test_agent_get_action_method(dummy_state: State,
    ...
    action_name = agent.get_action(dummy_state, [TEST_ACTION_NAME])

    assert action_name == TEST_ACTION_NAME
